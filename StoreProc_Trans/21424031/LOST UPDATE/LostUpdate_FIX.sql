...
CREATE 
PROC USP_21424031_CancelOrder_LU_Fix
	@OrderID AS INT
AS
BEGIN TRAN
	...
	--  Kiểm tra tồn tại thông tin đơn hàng và trạng thái đơn hàng là chờ vận chuyển
		IF EXISTS (SELECT * FROM Orders  WITH (XLOCK) WHERE ID = @OrderID AND Status = 1 AND DriverID is null)
		BEGIN
...
CREATE 
PROC USP_21424031_OrderConfirm_LU_Fix
	@OrderID AS INT,
	@DriverID AS INT
AS
BEGIN TRAN
